...
***We don't seem to have credentials for this , How do we login? (Images/There's something in the Resources Folder given to you )***
...
2. we have burp suite (Images/ we need to create a brute force attack )
...
![image.png](Images/609aeed8-63fa-4dfe-8718-366b7e8315a1.png)
...
In the android device go to Settings >  Security > Encryption & Credentials. > Install from SDCard (Images/ Install certificates from SD Card)
...
we need to complete the info, you need  (Images/ for my lab I sent this info ) and register
...
3. the info sent to vAPI (Images/ name, user, passsword, address, mobile )
...
Lets see we can see the user details, in the path /user/*(Images/id) and the correct token, maybe we can start to play with the path. 
...
token=$(Images/printf '%s:%s' "$user" "$pass" | base64 -w 0)
echo "$token"
...
talking about the CORS so we can out like header the parameter Origin :  [www.example.com](Images/http://www.example.com) and maybe we can capture the flag let´s do it.
...
Analysis: this is the clue<< **Nothing has been logged or monitored , You caught us :(Images/ ! >>**
